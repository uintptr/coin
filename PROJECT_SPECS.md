...
### 2.2a What a debate established

A debate's most valuable output is often a single concession made in round two
and scrolled off the screen by round five. The closing summary and
`transcript.md` therefore gather what the turns already recorded:

```
Findings
  A opened 85, ended 62   (-23)
  B opened 80, ended 45   (-35)

  A conceded the 2019 figure was superseded
  B conceded the effect size is smaller than I claimed

  A rests on the causal direction runs the other way
  B rests on the replication failed on the same data

  tools websearch 4, bash 2
```

**Nothing here is generated by a model.** Every line is computed from
[`TurnAnalysis`] values the debate already produced, so the summary is free,
instant, and cannot report a finding the debate did not reach. The judge pass
of section 4.5 is a separate thing: a verdict *about* the debate, which is
build step 10. These findings are the debate's own words, gathered.

Two details that matter for honesty:

- **Movement is measured between the first and last _readable_ readings**, not
  the first and last turns. A turn whose structured block could not be read
  states no confidence, and treating it as unchanged would invent a steadiness
  the side never expressed.
- **A single reading is reported as such**, not as a movement of zero. One
  stated confidence is a position; calling it no movement would read as a side
  that refused to budge, which is a different and damning claim.

What each side's case **finally** rests on is the last key claim it stated, not
the first. A side that revised what its argument depends on has told us
something, and the position that survived the debate is the interesting one.

The block is omitted entirely when a debate recorded none of this, so a failed
debate prints no empty heading.

### 2.2b What a debate spent is part of the result
